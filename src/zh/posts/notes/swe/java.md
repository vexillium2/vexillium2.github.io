...
Java9引入的`List.of()`, `Map.of()`, `Set.of()`用于创建不可变的、高效的、经过优化的集合实例。他们并非使用Array等创建，通过单例等进行内存优化，是不可变对象，不能使用常规集合方法。这样保证了线程安全。
...
**多态**=继承（重写）\+重载（开闭原则，面对抽象编程，不用修改父类，方便开发子类扩展功能）
...
在 Java 中，static 表示属于类而非实例，底层作为全局唯一存储，所有线程共享。它的诞生是在“一切皆对象”下，现实中存在大量无法被对应到实例的概念而不得不向面向过程做的妥协。在 JVM 中，静态字段存储在类的元数据结构里，先为静态字段开辟空间，再做初始化，初始化时线程同步加锁保证安全。static 的方法不可被继承/覆盖。

**静态代码块\(static block\)** 在类加载时执行，用于初始化静态变量 `static{ ... }`

用途：使用 `static final` 为常量修饰；作为天然线程安全的单例；类级别缓存；跨实例的计数器；工厂方法。

禁止：

实践例子：
- 线程安全的懒加载（使用内部静态类保证只有调用相关方法才加载，区别于饿汉式）
- 静态工厂方法
- 对象ID生成器（删减需考虑异常未正常减少的问题）

#### 其他关键词

**final** 修饰类时表示类不可继承不可覆写，修饰字段时表示不可修改（常量），用于写死相对路径、数字等。只表示"引用不可变"，不限制"对象不可变"。
...
**transient**序列化时忽略字段，如缓存、敏感信息。序列化指的是把对象变成字节流存到磁盘/网络传输，serialVersionUID 是给 JVM 序列化引擎读的元数据
...
Java程序运行是一个由编译器（javac）与虚拟机（JVM）精密协作的复杂生命周期。

**编译期：从源码到平台无关的字节码**

在编译阶段，javac编译器首先进行词法与语法分析，将代码解析为抽象语法树，随后进行严格的语义检查和类型校验。校验通过后，编译器会将高级语言.java翻译为平台无关的字节码指令（Bytecode），生成.class文件。在标准的工程构建流程中，这些.class文件会按照Java的包结构（相对路径）被输出至target/classes目录下，同时，resources目录下的配置文件等非代码资源也会被一并拷贝至此，形成完整的运行时类路径（Classpath）基础。
...
反射是 Java 动态编程的核心，它允许程序在运行期对任意类的内部结构进行探查与操作，即便在编译期对该类一无所知。通过 java.lang.reflect 包提供的 API，开发者可以绕过常规的访问控制，动态地实例化对象、调用方法以及修改字段。

#### Class 类

`Class` 类是反射的入口，它是 JVM 在加载类时生成的元数据容器。获取 `Class` 对象主要有三种途径：

1. 类字面量（`.class`）：如 `User.class`，在编译期确定，不会触发类的初始化。
2. 实例方法（`.getClass()`）：在已有对象实例时使用，返回运行时的实际类型。
3. 动态加载（`Class.forName("全限定名")`）：这是框架中最常用的方式，它不仅会加载类，还会立即触发该类的静态代码块执行，常用于驱动加载或配置驱动的类初始化。

通过 Class 对象，可以获取类的完整拓扑结构，包括 `getName()`（全限定名）、`getSuperclass()`（父类）、`getInterfaces()`（实现的接口）以及通过 `getResourceAsStream()` 读取类路径下的资源文件。
...
注解本质上是一种元数据（Metadata），它不直接参与业务逻辑的执行，而是作为标签附着在代码元素上，供编译器、构建工具或运行时框架读取并触发特定行为。注解的生命周期由 @Retention 元注解决定：SOURCE 仅保留在源码中，CLASS 保留在字节码中但 JVM 加载时丢弃，而 RUNTIME 则保留在字节码中且在运行时可通过反射读取。这是所有自定义业务注解（如 Spring 的 @Autowired、JPA 的 @Entity）必须采用的策略。注解本身不具备任何逻辑执行能力，框架通过反射 API 在运行时扫描类、方法或字段上的 RUNTIME 注解，提取属性值后，结合反射机制动态执行相应的增强逻辑。
...
面向抽象编程，工厂模式可以不用考虑创建实例的细节，调用方直接从工厂的接口中调用 getFactory() 获得工厂，或者在简单情况下，直接调用静态工厂方法获得实例。一个至于实际是从缓存中获得还是新建可以不关注，达到解耦效果。
...
通过许多小的构建步骤，构建一个复杂的对象。通过链式可以简化构建过程。
...
利用泛型实现适配器模式，将一个类的接口转换成另一个类的接口，使原本不兼容的类可以一起工作。

#### 代理（Proxy）

与适配器类似，但是类型不变，只是在运行时动态添加功能，能让调用方在不改变任何代码的前提下增强某些功能（例如，鉴权、延迟加载、连接池复用等）。。
...
享元可以理解为共享元对象，用于不可变的对象实例，注意单例只是不创建新的事例，而享元是共享对象。

Java 中的包装类型都是不变类，当传入-128到127之间的整数时，静态工厂方法 `Integer.valueOf()` 直接返回缓存好的实例。在实际应用中，享元模式主要应用于缓存，即客户端如果重复请求某些对象，不必每次查询数据库或者读取文件，而是直接返回内存中缓存的数据。Guava 的 Cache 是个成熟解决方案。

#### 组合（Composite）

Composite模式使得叶子对象和容器对象具有一致性，从而形成统一的树形结构，并用一致的方式去处理它们。例子：XML。
