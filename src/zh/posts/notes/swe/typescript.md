...
访问修饰符的演进经历了三个阶段。最初的代码仅依靠下划线前缀命名约定来区分内部接口，不具备任何强制约束力。TypeScript 引入的 `private` 关键字只在编译期生效，经过 `tsc` 转译后属性名不变，运行时仍然可以正常访问。ES2022 规范提供了以井号开头的原生私有字段，这种修饰在编译期与运行时双重生效，从语法层面阻止外部访问。团队开发中，面向内部业务代码使用 TypeScript `private` 即可满足协作要求；编写 SDK 或公共库时则需要使用原生私有字段来防止外部代码依赖内部实现。

### Interface 与类型谓词

TypeScript 的 `interface` 是纯编译期产物，转译后不会在生成的 JavaScript 代码中留下任何痕迹，这与 Java 接口编译后仍保留 `.class` 字节码并支持运行时反射有本质差别。因此运行时无法通过 `instanceof` 判断对象是否实现了某个 TypeScript 接口，必须借助类型谓词函数或 Schema 校验库完成运行时形态检查。
...
// 类型谓词：在运行时分支中让 TypeScript 自动收窄类型
