...
异步的表达形态决定了钩子回调的形态：callback 时代钩子以回调函数为接缝，Promise 时代接缝变为 `then` 链上的节点，async/await 时代钩子被封装进 await 两侧。这一演进是理解运行时 Hook 机制的伏笔。
...
单线程与事件循环回答了"代码何时跑"，Hook 机制回答下一个问题："在哪个时机插入什么"。前者的所有权在运行时，后者的注入权在业务代码，两者构成运行时叙事的完整闭环。

#### 原理：轻量 IoC 与开闭原则的运行时投影

扩展点（Hook）的本质是核心流程预留的"缝"。核心逻辑保持对扩展开放、对修改关闭——它不直接调用业务代码，而是维护一张回调注册表，在特定执行阶段遍历注册表并调用注册进来的函数。这是控制反转（IoC）的轻量级实现：流程的所有权在核心，业务代码只负责"在什么时候挂什么"，至于何时被触发，由核心流程说了算。

回调注册表在实现上有两种取向。观察者模式由被观察者直接维护订阅列表并逐一通知，耦合局限在同一个对象内部；发布-订阅模式则引入独立的事件通道，发布者与订阅者完全解耦、互不感知。Node.js 的 EventEmitter 介于两者之间：`on` 注册、`emit` 触发、`off` 摘除，天然适合进程内的事件流与多监听器并行消费。取舍标准是：需要临时挂载与摘除、需要多个监听器消费同一事件时用 EventEmitter；需要跨模块甚至跨进程解耦时，则引入独立的消息通道。

钩子的权力边界值得警惕：运行时与框架暴露的生命周期越完整，可注入能力越强，但被滥用的风险也越大。每一个扩展点都是一次隐式的全局行为注入，过度使用会让核心流程的"缝"变成"豁口"，行为的可预测性随之下降。

#### 形态一：生命周期钩子——进程与框架的生老病死

进程级生命周期钩子通过 `process.on` 挂载，是优雅关闭的最后防线。`SIGTERM` 与 `SIGINT` 处理函数负责停止接收新请求、等待在途请求完成、关闭连接池并 flush 日志后退出；`uncaughtException` 与 `unhandledRejection` 则兜住未捕获异常，保证进程在崩溃前有记录可查。
...
框架级生命周期钩子把"初始化时机"与"依赖注入的边界"绑定在一起。NestJS 的 `OnModuleInit`、`OnApplicationShutdown`，Egg 的 `didReady`，都是在模块加载、依赖解析完成后的固定时点被调用。此时注入的依赖已就绪、对外流量尚未开放，是建立连接池、加载配置、启动后台任务的正确窗口——早于这个时点依赖不可用，晚于这个时点则可能丢失首批请求。

#### 形态二：中间件钩子——请求管线的洋葱模型

中间件是请求生命周期上的 Hook。Express 的中间件是线性责任链，请求按注册顺序逐个穿越；Koa 的 `compose` 将其抽象为洋葱模型，每个中间件在 `next()` 前后的两段逻辑构成"进入时前置处理、返回时后置处理"的对称结构。两种模型遵循同一原则：中间件不关心请求的最终处理者是谁，只负责在穿越自己时完成切片式的横切关注点。

NestJS 将横切关注点进一步分层为 Guard、Interceptor、Pipe：Guard 负责鉴权，决定请求能否进入；Pipe 负责参数校验与转换，在进入处理器前修正数据；Interceptor 包裹整个处理过程，在调用前后执行日志、缓存、响应包装。这组分层把"鉴权 → 日志 → 校验 → 异常翻译"的职责拆到不同扩展点上，比在业务代码里堆叠条件判断干净得多。

实际案例覆盖请求耗时埋点、幂等控制、统一异常出口：耗时埋点利用洋葱模型的前后对称性计算区间，幂等控制在进入处理器前查询幂等键，统一异常出口在 `next()` 之后捕获处理器抛出的错误并翻译为统一响应结构。

```typescript
// 洋葱模型：进入时记录开始时间，返回时计算耗时
...
#### 形态三：异步内省钩子——异步世界的可观测性

`async_hooks` 是 Node.js 底层的异步内省机制，暴露异步资源的四个时机：`init`（创建）、`before`（回调执行前）、`after`（回调执行后）、`destroy`（销毁）。通过为每个异步资源分配 `asyncId` 并记录 `triggerAsyncId`（创建者的 ID），运行时得以重建完整的异步调用树——这些数据在普通代码里不可见，是夹在回调与回调之间的"暗物质"。

AsyncLocalStorage 建立在这棵异步调用树之上，实现跨异步边界的上下文传递，是请求级 tracing 的基石。在 `run()` 中存入 store 后，从该同步代码派生的所有异步操作——Promise 链、`setTimeout`、I/O 回调——都能通过 `getStore()` 读到同一份数据。它类比 Java 的 `ThreadLocal`，但"线程"换成了"异步资源链"：上下文精确绑定在调用链上，互不串扰。典型实践是在 HTTP 入口注入 `requestId`，此后该请求触发的所有日志、查询、外部调用共享同一标识符，实现全链路日志串联。
...
requestContext.run({ requestId: generateId() }, () => {
  handleRequest();
});

function handleRequest() {
  const { requestId } = requestContext.getStore() ?? {};
  logger.info(`处理请求 ${requestId}`);
}
```

diagnostics_channel 提供零侵入的发布/订阅通道：进程内任意模块可以向命名频道发布消息，观测方按需订阅，被观测代码完全无感。相比直接打日志，它把"观测"与"业务"彻底分离，适合在库内部埋点、由运维侧决定是否采集。

#### 实践反思："魔法"的代价

Hook、中间件、依赖注入容器本质是同一族思想——控制反转——的不同强度。Hook 让核心流程在固定时点调用注册回调；中间件把注入点标准化为请求管线的固定位置；DI 容器则接管对象的创建与装配。强度越高，业务代码越"干净"，但行为的隐式程度也越高。

Hook 的另一面是隐式耦合：回调的执行时机由运行时决定，阅读者无法从调用栈直接看出谁注册了什么；多个钩子之间的执行顺序构成隐式契约，顺序假设一旦被打破，行为难以定位；异步钩子还会切断调用栈，异常堆栈里看不到"谁在何时注册了这段逻辑"。这些代价决定了 Hook 应当服务于横切关注点，而不是替代显式的函数调用。

TS 在其中的角色是给扩展点加上类型：typed event map 让事件名与回调签名在编译期对齐；Koa 的 `Middleware` 泛型约束中间件的上下文与返回类型；NestJS 的装饰器配合 `reflect-metadata` 把 AOP 元数据变成可检查的类型信息。类型把"魔法"约束在可预期的边界内——这一叶将 Hook 拉回 TypeScript 类型系统的主线（呼应类型系统中预留的 "TS 对运行时的间接影响"）。
